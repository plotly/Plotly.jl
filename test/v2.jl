...
using Base.Test, Requests

M = PlotlyV2  # simplify name
...
@testset "api" begin
    ...
    @testset "files" begin
        ...
        res = Requests.json(res_star_raw)
    ...
    @testset "grids" begin
        # TODO: need to test grid_put_col, grid_post_col, grid_drop_reference,
        #       grid_permanent_delete, grid_upload

        fid = "sglyon:251"
        uid1 = "d6434f"
        uid2 = "8c7b60"

        res_retrieve = PlotlyV2.grid_retrieve_raw(fid)
        res_retrieve.headers["Content-Type"]
        @test 200 == statuscode(res_retrieve)
        json_retrieve = Requests.json(res_retrieve)
        ...
        res = PlotlyV2.grid_create_raw(data, parent_path="_jl_testing", filename=del_fn)
        json_delete = Requests.json(res)
        ...
        col_json = Requests.json(res_col)
        ...
        lookup_json = Requests.json(res_lookup)
